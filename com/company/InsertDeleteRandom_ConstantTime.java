 ...
 */
public class InsertDeleteRandom_ConstantTime {
    Map<Integer,Integer> hashMap = new HashMap<>();
    List<Integer> arrayList;
    Random random;
    public InsertDeleteRandom_ConstantTime() {
        hashMap = new HashMap<>();
        random = new Random();
    ...
    public boolean remove(int val) {
        ...
        if(valIndex != arrayList.size()-1){
            int lastElement = arrayList.get(arrayList.size()-1);
            int lastIndex = arrayList.size()-1;
            arrayList.set(valIndex,lastElement);
            hashMap.put(lastElement,valIndex);
            arrayList.set(lastIndex,val);
        }
        hashMap.remove(val);
